...
export function createMoveChange(editor: Editor, blockId: string): DocumentChange {
  let blockEntry = getBlock(editor, { id: blockId })

  if (blockEntry) {
    let [, path] = blockEntry
    let parent = path.length == 2 ? '' : (Node.parent(editor, Path.parent(path)) as FlowContent).id
    let leftSibling = path[path.length - 1] == 0 ? '' : (Node.get(editor, Path.previous(path)) as FlowContent).id
...
export function createReplaceChange(editor: Editor, blockId: string): DocumentChange {
  let blockEntry = getBlock(editor, { id: blockId })
