...
export function SidePanelObject(props: any) {
  ...
  return (
    <li
      ...
      className="relative m-4 break-words whitespace-pre-wrap bg-white border rounded-lg border-muted"
    >
      <div className="p-4">
        <div className="flex items-center justify-between text-muted-hover">
          <p className="text-xs uppercase text-muted-hover">Document</p>
          <div>
            <button
              onClick={() => setOpen((val) => !val)}
              className="p-1 transition duration-100 rounded hover:bg-background-muted hover:text-body-muted"
            >
              {/* // TODO: add icons */}
              {/* {open ? (
        ...
        <h2 className="mt-2 font-bold">{data?.document?.title}</h2>
...
function ObjectFooter({ version }: any) {
  return (
    <div className="border-t">
      <Link
        to={`/p/${version}`}
        className="flex items-center p-4 text-sm font-bold text-primary hover:bg-background-muted"
      >
        {/* TODO: add icon */}
    ...
    ({ children, ...props }) => {
      switch (props.element.type) {
        ...
        case ELEMENT_READ_ONLY:
          return (
            <div className="px-2 mt-1 -mx-2 rounded bg-background" {...props}>
              {children}
            </div>
          );
