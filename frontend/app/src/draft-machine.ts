      ...
      actions: {
        ...
        assignDraftsValue: assign((_, event) => {
          ...
          if (event.data.children?.length) {
            ...
            MintterEditor.addChange(editor, ['moveBlock', entryNode.id])
            MintterEditor.addChange(editor, ['replaceBlock', entryNode.id])
          }
        ...
        saveDraft: (context) => (sendBack) => {
          if (shouldAutosave) {
            ;(async function autosave() {
              ...
              debug(
                '🚀 ~ file: draft-machine.ts ~ line 330 ~ autosave ~ contentChanges',
                contentChanges,
              )
