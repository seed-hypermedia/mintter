      ...
      <div className="bg-background p-8 rounded-2xl shadow-lg outline-none focus:shadow-outline">
        ...
        <div className="mt-8 border-t pt-8">
          <h2 className="text-2xl font-light">
            {`Keep this information handy, in order to access ${document.author?.username}’s document
              in the Mintter App.`}
          </h2>
          <div className="mt-6 flex items-center">
            <CopyToClipboard
              ...
              onCopy={(_, result) => {
                if (result) {
                  addToast(
                    `${document.author.username}’s Address copied to your clipboard!`,
                    {
                      appearance: 'success',
                    },
                  )
            ...
            >
              <button className="outline-none focus:shadow-outline text-primary pl-2 pr-4 py-1 font-bold flex items-center rounded-full border-2 border-primary hover:bg-primary hover:text-white transition duration-100">
                ...
                <span className="ml-2">{`Copy ${document.author?.username}’s user ID`}</span>
              </button>
            </CopyToClipboard>
