...
export default function Steps({steps, active}: Props) {
  return (
    <div className="w-full max-w-3xl mx-auto mt-8 sm:mb-8 py-8 px-16">
      <ol
        className={`steps-wrapper flex w-full items-center justify-between relative ${css(
          {
            '&::before': {
              content: '""',
              position: 'absolute',
        ...
        {steps &&
          steps.map((step, index) => (
            <li key={step.title} className="flex flex-col relative">
              <span
                className={`relative w-4 h-4 rounded-full flex items-center justify-center text-muted ${
                  active === index
