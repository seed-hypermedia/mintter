...
export type AccountsDefinition = typeof AccountsDefinition;
export const AccountsDefinition = {
  name: "Accounts",
  fullName: "com.mintter.accounts.v1alpha.Accounts",
  methods: {
    /**
...
export interface AccountsServiceImplementation<CallContextExt = {}> {
  ...
  /** Update Profile information of our own Account. */
  updateProfile(request: Profile, context: CallContext & CallContextExt): Promise<DeepPartial<Account>>;
  /**
   * List accounts known to the backend (excluding our own account). New accounts can be discovered naturally by
   * interacting with the network, or users can ask to discover specific accounts using
   * the Networking API.
...
export interface AccountsClient<CallOptionsExt = {}> {
  /**
   * Lookup an Account information across the already known accounts.
   * Can also be used to retrieve our own account.
   */
  ...
  /** Update Profile information of our own Account. */
  updateProfile(request: DeepPartial<Profile>, options?: CallOptions & CallOptionsExt): Promise<Account>;
  /**
   * List accounts known to the backend (excluding our own account). New accounts can be discovered naturally by
   * interacting with the network, or users can ask to discover specific accounts using
   * the Networking API.
