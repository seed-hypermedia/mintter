...
import { BUNDLED_LANGUAGES, getHighlighter, Highlighter, Lang, setCDN } from 'shiki'
    ...
    decorate:
      ...
      ([node, path]) => {
        ...
        if (isParagraph(node)) {
          ...
          const lines = (code.data?.[HIGHLIGHTER] as Highlighter).codeToThemedTokens(
            string,
            code.lang,
            code.data.theme as string,
            {
...
function Code({
  ...
  useEffect(() => {
    ...
    Transforms.setNodes(editor, { data: { ...newData, theme: codeTheme } }, { at: path })
  }, [theme, editor])
