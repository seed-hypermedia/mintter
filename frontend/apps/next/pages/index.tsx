...
let pubId =
  process.env.MINTTER_HOME_PUBID || 'bafy2bzacea346azbi4r5fxebdvz6wpkak7ati3cf5vywtruw4aabjeoi2332w'
let version =
...
function DefaultHomePage({ siteInfo }: { siteInfo: SiteInfo | null }) {
  let { data } = useQuery({
    ...
    queryFn: () => publicationsClient.getPublication({ documentId: pubId, version }),
  })
  if (data) {
    return <PublicationPage publication={data} metadata={false} siteInfo={siteInfo} />
  }
  ...
  return (
    <>
      ...
      <main id="main-content" tabIndex={-1} className="main-content wrapper text-size-100">
