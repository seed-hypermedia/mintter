import {isLink, isPhrasingContent, isText} from '../../assertions'
import {paragraph} from '../../builder'
import {MttastNode, PhrasingContent} from '../../types'
...
function runs(
  ...
  while (++index < flattened.length) {
    ...
    if (isPhrasingContent<PhrasingContent>(node)) {
      if (!queue) queue = []
      queue.push(node)
    } else {
      if (queue) {
