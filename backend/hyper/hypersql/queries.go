...
func generateQueries() error {
	code, err := sgen.CodegenQueries("hypersql",
		...
		qb.MakeQuery(s.Schema, "ChangesListFromChangeSet", sgen.QueryKindMany,
			...
			"WHERE", s.HyperChangesViewEntity, "=", qb.VarCol(s.HyperChangesViewEntity), '\n',
			"AND", s.HyperChangesViewBlobID, "= cset.value", '\n',
		...
		qb.MakeQuery(s.Schema, "ChangesListPublicNoData", sgen.QueryKindMany,
			"SELECT", qb.Results(
				...
				s.HyperChangesViewMultihash,
				s.HyperChangesViewSize,
				s.HyperChangesViewEntity,
				s.HyperDraftsBlob,
			), '\n',
			"FROM", s.HyperChangesView, '\n',
			"LEFT JOIN", s.HyperDrafts, "ON", s.HyperDraftsEntity, "=", s.HyperChangesViewEntityID, '\n',
			"WHERE", s.HyperDraftsBlob, "IS NULL", '\n',
			"ORDER BY", qb.Enumeration(s.HyperChangesViewEntity, s.HyperChangesViewHlcTime),
		),
