// Package threadsutil provides some helper functions for working with github.com/textileio/go-threads.
package threadsutil
...
func DefaultService(repoPath string, peerKey crypto.PrivKey, opts ...ServiceOption) (sb *ServiceBootstrapper, err error) {
	...
	if peerKey == nil {
		peerKey = util.LoadKey(filepath.Join(ipfsLitePath, "key"))
	}
