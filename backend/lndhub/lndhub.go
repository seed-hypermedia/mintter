	...
	if json_data, err := json.Marshal(payload); err != nil {
		...
		if resp, err := client.Do(req); err != nil {
			...
			if resp.StatusCode != http.StatusOK {
				...
				return invoice, fmt.Errorf("could't create invoice, error code " + code +
					" message " + message)
...
// enconded amount in the invoid should match the provided amount as a double check
func (l *Lndhub) PayInvoice(pay_req string, sats uint64) error {

	var ret map[string]interface{}
	payload := map[string]string{"invoice": pay_req}

	if invoice, err := l.DecodeInvoice(pay_req); err != nil {
		return nil
	...
	} else if json_data, err := json.Marshal(payload); err != nil {
		return err
	} else if req, err := http.NewRequest("POST", l.apiurl+payInvoiceRoute, bytes.NewBuffer(json_data)); err != nil {
		return err
	} else {
		// add authorization header to the req
		req.Header.Add("Authorization", "Bearer "+l.access_token)
		...
		if resp, err := client.Do(req); err != nil {
			...
			defer resp.Body.Close()
			if resp.StatusCode != http.StatusOK {
				return fmt.Errorf("Response status " + http.StatusText(resp.StatusCode) +
					"expected " + http.StatusText(http.StatusOK))
			} else if err := json.NewDecoder(resp.Body).Decode(&ret); err != nil {
				return err
			} else if err_field, ok := ret["error"]; ok && err_field.(bool) {
				...
				return fmt.Errorf("could't pay invoice, error code " + code +
					" message " + message)
