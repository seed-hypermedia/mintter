...
func main() {
	...
	siteTimeout := flag.Duration("site-timeout", time.Duration(10*time.Second), "Timeout to consider the site to be offline.")
	numPings := flag.Int("num-pings", 5, "Number of pings per site to calculate average TTR.")
	port := flag.Int("port", 5002, "Port to run the server on.")
	templateFile := flag.String("html-template", "./template.html", "Path of the htl template file.")
	flag.Parse()
	if *siteTimeout > *scanPeriod {
	...
	srv, err := server.NewServer(*port, *numPings, *scanPeriod, *siteTimeout, *templateFile, log.Desugar(), sitesList...)
	if err != nil {
		panic(err)
	}
