...
func TestTransclusionEmptyDraft(t *testing.T) {
	srv, _, ctx := makeServer(t, "alice")
	...
	draft2, err := srv.CreateDraft(ctx, &v2.CreateDraftRequest{})
	require.NoError(t, err, "must create draft 2")
	draft2.Title = "Document with only transcluded blocks"
	draft2.BlockRefList = &v2.BlockRefList{
		Refs: []*v2.BlockRef{
			{
				Ref: pub1.Version + "/block-list-parent",
				BlockRefList: &v2.BlockRefList{
	...
	updateReq2 := &v2.UpdateDraftRequest{Document: draft2}
