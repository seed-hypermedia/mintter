...
impl<L: ModuleLoader> Host<L> {
        ...
        R: DeserializeOwned + Debug,
    {
        ...
        let futs = exts.map(|(_, ext)| Box::pin(ext.call(hook, payload.clone())));
        let (output, _) = futures_util::future::select_ok(futs).await?;

        let result = serde_json::from_slice(&output).map_err(Into::into);
        ...
        R: DeserializeOwned + Debug,
    {
        ...
        let futs = exts.map(|(_, ext)| {
            let fut = ext.call(hook, payload.clone()).and_then(|out| async move {
                serde_json::from_slice::<R>(&out).map_err(Into::into)
            });
        ...
        let out = futures_util::future::join_all(futs)
            .await
            .into_iter()
            .inspect(|el| debug!(target = "hook_parallel", hook = ?hook, plugin_result = ?el, "Plugin result"))
