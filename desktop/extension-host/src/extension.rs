...
impl Extension {
  ...
  pub fn new<L: ModuleLoader>(host: &Host<L>, bytes: &[u8], id: usize) -> Result<Self, Error> {
    ...
    debug!(
      target = "new",
      "Collecting information about exported funcs"
    );
    let exports = module
      .exports()
      .filter_map(|exp| {
...
async fn call_hook(ext: Extension, hook: Hook, payload: Vec<u8>) -> Result<Vec<u8>, Error> {
    ...
    .and_then(|e| e.into_func())
  {
    debug!(
      target = "call_hook",
      "Initialize wasm module state (`_start` func)"
    );
    let start: TypedFunc<(), ()> = start.typed(&mut store)?;
    ...
    debug!(
      target = "call_hook",
      "Skipping initialization: Wasm module doesn't export `_start`"
    );
  };
